...
=== TEST 8: add plugin with 'include_req_body' setting
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 9: verify plugin for file-logger with request
--- config
    location /t {
        content_by_lua_block {
            local core = require("apisix.core")
...
=== TEST 10: check file-logger 'include_req_body' with 'expr'
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 11: verify file-logger req with expression of concern
--- config
    location /t {
        content_by_lua_block {
            local core = require("apisix.core")
...
=== TEST 12: Add new configuration with match
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
