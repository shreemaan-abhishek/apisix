...
=== TEST 3: create only one healthcheck instance
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
            local core = require("apisix.core")
            local send_request = function(apikey)
            ...
            for i = 1, 10 do
                local th = assert(ngx.thread.spawn(function(i)
                    for j = 1, 20 do
                       send_request(string.format("auth-one%d", i))
                    end
                end, i))
                table.insert(ths, th)
            end
            for i, th in ipairs(ths) do
                ngx.thread.wait(th)
            end
            ngx.say("passed")
        }
