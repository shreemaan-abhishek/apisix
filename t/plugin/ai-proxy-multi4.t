...
=== TEST 1: create a ai-proxy-multi plugin that use post method as health check
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 2: initialize the node list once in 4-worker
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
passed
--- grep_error_log eval
qr/adding an existing target/
--- grep_error_log_out
adding an existing target
adding an existing target
adding an existing target
