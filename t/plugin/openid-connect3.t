...
=== TEST 5: Set an unsupported scope in the required scopes field
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 7: Set a supported scope in the required scopes field
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 12: Update route with Keycloak introspection endpoint and introspection addon headers.
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 13: Obtain valid token and access route with it, introspection work as expected when configured extras headers.
--- config
    location /t {
        content_by_lua_block {
            -- Obtain valid access token from Keycloak using known username and password.
...
=== TEST 14: Access route with an invalid token, should fail.
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 15: Update route with fake Keycloak introspection endpoint and introspection addon headers
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 16: Check http headers from fake introspection endpoint.
--- config
    location /t {
        content_by_lua_block {
            local http = require "resty.http"
