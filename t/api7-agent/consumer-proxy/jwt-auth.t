        ...
        local auth_plugin_payload = {
            ["jwt-auth"] = {
                ["jwt-one"] = {
                    ...
                    username = "test",
                    plugins = {},
                    credential_id = "47a8e18d-6f81-4f3f-a069-38459d8ca93b",
                    auth_conf = {
                        algorithm = "HS256",
                        exp = 315569260,
                        lifetime_grace_period = 0,
                        secret = "alsrjPvz8hSeEqqMzVdFI3FSIZqzDN91DWPdOH21+OQ=",
                        key = "jwt-one"
                    }
                }
            }
...
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
            local code, err, sign = t('/apisix/plugin/jwt/sign?key=jwt-one',
                ngx.HTTP_GET
            )
