...
=== TEST 8: client request with trailing period "."
--- config
listen unix:$TEST_NGINX_HTML_DIR/nginx.sock ssl;
location /t {
    content_by_lua_block {
...
=== TEST 9: set ssl(snis: {"www.test.com."})
--- config
location /t {
    content_by_lua_block {
        local core = require("apisix.core")
        ...
        local ssl_cert = t.read_file("t/certs/test-dot.crt")
        local ssl_key =  t.read_file("t/certs/test-dot.key")
        local data = {cert = ssl_cert, key = ssl_key, snis = {"www.test.com."}}
        local code, body = t.test('/apisix/admin/ssls/1',
            ngx.HTTP_PUT,
            core.json.encode(data),
            [[{
