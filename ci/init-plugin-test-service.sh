...
after() {
    ...
    docker exec -i apache-apisix_kafka-server3-scram_1 /opt/bitnami/kafka/bin/kafka-topics.sh --create --zookeeper zookeeper-server3:2181 --replication-factor 1 --partitions 1 --topic test-scram-256
    docker exec -i apache-apisix_kafka-server3-scram_1 /opt/bitnami/kafka/bin/kafka-topics.sh --create --zookeeper zookeeper-server3:2181 --replication-factor 1 --partitions 1 --topic test-scram-512
  # Create user with SCRAM-SHA-512
  docker exec apache-apisix_kafka-server3-scram_1 /opt/bitnami/kafka/bin/kafka-configs.sh \
    ...
    chmod +x jq
    docker cp jq apisix_keycloak_new:/usr/bin/
