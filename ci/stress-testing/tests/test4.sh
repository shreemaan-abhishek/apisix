...
TEST_NAME="Case 4: Single core, Service + upstream, forced internal 500, torture testing for one minute"
echo "-----------Running test 4: $TEST_NAME ---------------"
  ...
  -X PUT -d '
{
  ...
  "plugins": {
      "serverless-pre-function": {
          "phase": "rewrite",
          "functions" : ["return function() error('test') end"]
      }
  }
}'
...
# Check if jq command was successful before replacing the original file
if [ $? -eq 0 ]; then
