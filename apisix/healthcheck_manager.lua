...
local disable_upstream_healthcheck
...
function _M.get_target_list(value)
    if not healthcheck then
        healthcheck = require("resty.healthcheck")
    end

    if disable_upstream_healthcheck then
        return
    end

    local name = get_healthchecker_name(value)
    return healthcheck.get_target_list(name, "upstream-healthcheck")
end
