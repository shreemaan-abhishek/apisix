...
local function init_iv_tbl(ivs)
    local _aes_cbc_with_iv_tbl = core.table.new(2, 0)
    local type_ivs = type(ivs)

    local values = ivs
    if type_ivs == "string" then
        values = {ivs}
    end

    if type(values) == "table" then
        for _, iv in ipairs(values) do
            local aes_with_iv
            if #iv == 32 then
                aes_with_iv = assert(aes:new(iv, nil, aes.cipher(256, "cbc"), {iv = iv}))
            elseif #iv == 16 then
                aes_with_iv = assert(aes:new(iv, nil, aes.cipher(128, "cbc"), {iv = iv}))
            end

            core.table.insert(_aes_cbc_with_iv_tbl, aes_with_iv)
        end
    end

    return _aes_cbc_with_iv_tbl
end


local _aes_cbc_with_iv_tbl_ssl
local function get_aes_cbc_with_iv_ssl(local_conf)
    if _aes_cbc_with_iv_tbl_ssl == nil then
        local ivs = core.table.try_read_attr(local_conf, "apisix", "ssl", "key_encrypt_salt")
        _aes_cbc_with_iv_tbl_ssl = init_iv_tbl(ivs)
    end

    return _aes_cbc_with_iv_tbl_ssl
end


local _aes_cbc_with_iv_tbl_gde
local function get_aes_cbc_with_iv_gde(local_conf)
    if _aes_cbc_with_iv_tbl_gde == nil then
        local ivs = core.table.try_read_attr(local_conf, "apisix", "data_encryption", "keyring")
        _aes_cbc_with_iv_tbl_gde = init_iv_tbl(ivs)
    end

    return _aes_cbc_with_iv_tbl_gde
end



local function encrypt(aes_cbc_with_iv, origin)
    local encrypted = aes_cbc_with_iv:encrypt(origin)
...
function _M.aes_encrypt_pkey(origin, field)
    ...
    if not field then
        -- default used by ssl
        local aes_cbc_with_iv_tbl_ssl = get_aes_cbc_with_iv_ssl(local_conf)
        local aes_cbc_with_iv_ssl = aes_cbc_with_iv_tbl_ssl[1]
        if aes_cbc_with_iv_ssl ~= nil and core.string.has_prefix(origin, "---") then
            return encrypt(aes_cbc_with_iv_ssl, origin)
        end
    else
        if field == "data_encrypt" then
            local aes_cbc_with_iv_tbl_gde = get_aes_cbc_with_iv_gde(local_conf)
            local aes_cbc_with_iv_gde = aes_cbc_with_iv_tbl_gde[1]
            if aes_cbc_with_iv_gde ~= nil then
                return encrypt(aes_cbc_with_iv_gde, origin)
            end
        end
    end
...
local function aes_decrypt_pkey(origin, field)
    local local_conf = core.config.local_conf()
    local aes_cbc_with_iv_tbl

    if not field then
        if core.string.has_prefix(origin, "---") then
            return origin
        end
        aes_cbc_with_iv_tbl = get_aes_cbc_with_iv_ssl(local_conf)
    else
        if field == "data_encrypt" then
            aes_cbc_with_iv_tbl = get_aes_cbc_with_iv_gde(local_conf)
        end
    end

    if #aes_cbc_with_iv_tbl == 0 then
        return origin
    end
    ...
    for _, aes_cbc_with_iv in ipairs(aes_cbc_with_iv_tbl) do
        local decrypted = aes_cbc_with_iv:decrypt(decoded_key)
        if decrypted then
            return decrypted
        end
    end
